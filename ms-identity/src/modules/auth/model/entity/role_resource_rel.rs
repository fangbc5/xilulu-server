/// RoleResourceRel
/// 
/// 表名: `role_resource_rel`
/// 主键: `id`
/// 逻辑删除字段: `is_del`
/// 字段数: 11

#[derive(Debug, Default, sqlx::FromRow, serde::Serialize, serde::Deserialize, sqlxplus::ModelMeta, sqlxplus::CRUD)]
#[model(table = "role_resource_rel", pk = "id", soft_delete = "is_del", table_comment = "角色的资源")]
pub struct RoleResourceRel {
    /// 主键 | id (bigint) | 非空
    #[column(primary_key, auto_increment, comment = "主键")]
    pub id: Option<i64>,
    /// resource_id (bigint) | 非空
    #[column(not_null, unique, index, comment = "拥有资源;#def_resource")]
    pub resource_id: i64,
    /// application_id (bigint) | 非空
    #[column(not_null, comment = "所属应用;#def_application")]
    pub application_id: i64,
    /// role_id (bigint) | 非空
    #[column(not_null, unique, index, comment = "所属角色;#base_role")]
    pub role_id: i64,
    /// create_time (timestamp(3)) | 可空
    /// 默认值: CURRENT_TIMESTAMP(3)
    #[column(default = "CURRENT_TIMESTAMP(3)", length = 3, comment = "创建时间")]
    pub create_time: Option<chrono::DateTime<chrono::Utc>>,
    /// create_by (bigint) | 可空
    #[column(comment = "创建人")]
    pub create_by: Option<i64>,
    /// update_time (timestamp(3)) | 可空
    /// 默认值: CURRENT_TIMESTAMP(3)
    #[column(default = "CURRENT_TIMESTAMP(3)", length = 3, comment = "最后更新时间")]
    pub update_time: Option<chrono::DateTime<chrono::Utc>>,
    /// update_by (bigint) | 可空
    #[column(comment = "最后更新人")]
    pub update_by: Option<i64>,
    /// created_org_id (bigint) | 可空
    #[column(comment = "创建人组织")]
    pub created_org_id: Option<i64>,
    /// is_del (tinyint(1)) | 非空
    /// 默认值: 0
    #[column(not_null, default = "0", length = 1, soft_delete, comment = "")]
    pub is_del: Option<bool>,
    /// tenant_id (bigint) | 非空
    #[column(not_null, comment = "")]
    pub tenant_id: i64,
}
