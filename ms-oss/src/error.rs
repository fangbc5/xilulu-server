...
impl IntoResponse for OssError {
    fn into_response(self) -> Response {
        let (status, err_code, message) = match &self {
            OssError::FileNotFound(msg) => (StatusCode::NOT_FOUND, code::FILE_NOT_FOUND, msg.clone()),
            OssError::FileSizeMismatch(msg) => (StatusCode::BAD_REQUEST, code::FILE_SIZE_MISMATCH, msg.clone()),
            OssError::PresignFailed(msg) => (StatusCode::INTERNAL_SERVER_ERROR, code::PRESIGN_FAILED, msg.clone()),
            OssError::CallbackFailed(msg) => (StatusCode::INTERNAL_SERVER_ERROR, code::CALLBACK_FAILED, msg.clone()),
            OssError::BadRequest(msg) => (StatusCode::BAD_REQUEST, code::BAD_REQUEST, msg.clone()),
            OssError::FileTypeNotAllowed(msg) => (StatusCode::BAD_REQUEST, code::FILE_TYPE_NOT_ALLOWED, msg.clone()),
            OssError::FileTooLarge(msg) => (StatusCode::BAD_REQUEST, code::FILE_TOO_LARGE, msg.clone()),
            OssError::InternalError(msg) => (StatusCode::INTERNAL_SERVER_ERROR, code::INTERNAL_ERROR, msg.clone()),
        };
